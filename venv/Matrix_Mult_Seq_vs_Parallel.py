...
def par_matrix_multiply(A, B):
    ...
    if num_rows_A * num_cols_B < 25_000:
        print("We are using sequetial version")
        return seq_matrix_multiply(A, B)

    """  _______________________   Partitioning  ________________________        """

    # create workers to calculate results for subset of rows in C.
    num_workers = mp.cpu_count()  # return the number of available processors.
    # split chunks across available processors.
    chunk_size = math.ceil(num_rows_A / num_workers)  # num_rows_A is the same as num_rows_C
    # specify shared Array which their elements are asynchronous.
    C_1D = mp.RawArray('d', num_rows_A * num_cols_B)  # flat version of matrix C.
    workers = []  # empty list to collect workers in on pool.
    for w in range(num_workers):
        row_start_C = min(w * chunk_size, num_rows_A)  # specifying the beginning of insertion.
        row_end_C = min((w + 1) * chunk_size, num_rows_A)  # the end of the line .
